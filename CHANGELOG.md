...
## The colregs section had no colour because it was not a picture

Three rounds of "still no color in colregs" and I checked the wrong thing
every time. The plate in the `## colregs` section was a plain fenced text
block, not an SVG. Only the one in "Colour and night vision" was ever an
image, and that one had been correct throughout. Larry was looking at the
text block; I kept examining the picture.

- The colregs section now shows `colregs-mineclear.svg` - the mine clearance
  vessel, three all-round greens on a mast and a yard - in colour, with the
  copyable text kept in a `<details>` block like the other sections. That
  file was already being generated by `make-site.sh` and used nowhere.
- The tides section now shows `tides-find.svg`, likewise already generated
  and unused.
- **`tests/plates-check.sh`**: the README pastes real program output into
  fenced blocks by hand, so it drifts. The colregs plate had been sitting
  there saying "You are on her starboard bow" long after the program started
  saying "broad on her starboard bow", and without the note explaining the
  sidelight geometry. The check regenerates each plate and diffs it.

## The sidelight that looks as though it is on the wrong side

Larry asked twice - once of the towing plate, once of the vessel restricted
in her ability to manoeuvre - why a green sidelight is drawn *aft* of the
masthead lights when you are on her starboard bow. Intuition says a sidelight
is forward, so it should be toward the bow.

The drawing is right, and there are two reasons:

- **Annex I 3(b)**: "On a power-driven vessel of 20 metres or more in length
  the sidelights shall not be placed in front of the forward masthead
  lights." Aft of the masts is where they belong.
- At a bow aspect you are looking **across her beam more than along her
  length**. At 040 the athwartships offset is weighted by cos 40 (0.77) and
  the fore-and-aft offset by sin 40 (0.64), so her beam wins - and her
  starboard side swings toward *your* left. It is the same geometry as
  bow-on, where her green is on your left and her red on your right; it just
  fades out as she comes beam-on.

The note under the drawing now says both. `tests/annex1-check.awk` gained the
other half of 3(b): a sidelight forward of the forward masthead light now
fails. The projection was verified against an independently constructed
observer/line-of-sight/cross-product model rather than by re-deriving the
same formula.
